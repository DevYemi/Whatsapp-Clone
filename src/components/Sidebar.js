...
import { getChatsFromDb, getRoomsFromDb, getUserInfoFromDb } from './get&SetDataToDb';
...
function Sidebar() {
    ...
    useEffect(()=>{ // on first render display the first convo on the chat div
        if (convos.length > 1 && isFirstRender) {
            let firstConvo = convos[0]
            let location = firstConvo.data.isRoom ? `/rooms/${firstConvo.id}` : `/chats/${firstConvo.id}`
